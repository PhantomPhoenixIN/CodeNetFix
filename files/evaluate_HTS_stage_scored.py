...
- Error-type fixing analysis
- Stage-wise performance
- Continuous stage_score analysis
...
INPUT_JSONL = "/home/swaminathanj/TransRectify_New/NMT_Rectifier/content/datasets/rectifier_predictions/CodeNetFix_RF_test_HT_shuffled_greedy_with_stage.jsonl"
...
def evaluate():
    ...
    with open(INPUT_JSONL, "r") as fin:
        ...
        for idx, line in enumerate(fin, 1):
            ...
            stage_score = item.get("stage_score", None)
            ...
            if functional:
                ...
                if stage_score is not None:
                    stage_score_success.append(stage_score)

            if stage_score is not None:
                stage_score_all.append(stage_score)
    ...
    for lang in sorted(lang_stats.keys()):
        ...
        print(
            f"{lang:<10} "
            f"Total={t} | "
            f"Parsable={s['parsable']} ({s['parsable']/t*100:.2f}%) | "
            f"Compilable={s['compilable']} ({s['compilable']/t*100:.2f}%) | "
            f"Pass@1={s['pass@1']} ({s['pass@1']/t*100:.2f}%)"
        )
    ...
    for stage in sorted(stage_stats.keys()):
        ...
        if t == 0:
            continue
        print(f"{stage:<15} Total={t} | Pass@1={s['pass@1']} ({s['pass@1']/t*100:.2f}%)")
    ...
    if stage_score_all:
        print("\nContinuous Stage Score Analysis")
        print(f"Avg stage_score (all): {sum(stage_score_all)/len(stage_score_all):.4f}")
        if stage_score_success:
            print(f"Avg stage_score (successful): {sum(stage_score_success)/len(stage_score_success):.4f}")
    ...
    final_results = {
        ...
        "avg_stage_score_all": (
            sum(stage_score_all)/len(stage_score_all)
            if stage_score_all else None
        ),
        "avg_stage_score_success": (
            sum(stage_score_success)/len(stage_score_success)
            if stage_score_success else None
        )
    }
